...
class DB:
	...
	def populate(self):
		...
		cur.execute('INSERT INTO users (UUID,first_name,last_name,dob,city,emirate,po_box,address_1,address_2,phone,email,password,isAdmin,isVerified) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)', (jake_uuid, "Jake", "Peralta", "20/6/1981", "Al Ain", "Abu Dhabi", "33344", "An awesome police station.", "A really noice police station.", "050ILOVEAMY", "jake@email.com", passwd, 0, 1))
		dbcon.commit()
		...
		# books_pic = b64(open('./assets/shirt.jpg', 'rb').read())
		# tools_pic = b64(open('./assets/shirt.jpg', 'rb').read())
		# lego_set_pic = b64(open('./assets/shirt.jpg', 'rb').read())
		# bedsheets_pic = b64(open('./assets/shirt.jpg', 'rb').read())
		# children_socks_pic = b64(open('./assets/shirt.jpg', 'rb').read())
		# barbie_set_pic = b64(open('./assets/shirt.jpg', 'rb').read())
		# software_engineering_textbook_pic = b64(open('./assets/shirt.jpg', 'rb').read())
		...
		cur.execute('INSERT INTO items (UUID,item_name,category,condition,description,org_id,user_id,time_submitted,pickup_time,image,status) VALUES (?,?,?,?,?,?,?,?,?,?,1)', (books_uuid,'Children\'s Books','Books','Lightly used','A children\'s book about the tooth fairy.',red_cresent_uuid,donald_uuid,datetime.now().strftime("%d/%m/%Y - %H:%M:%S"),'Morning (8:00 AM - 11:00 AM)',b''))
		cur.execute('INSERT INTO items (UUID,item_name,category,condition,description,org_id,user_id,time_submitted,pickup_time,image,status) VALUES (?,?,?,?,?,?,?,?,?,?,-1)', (tools_uuid,'Construction Tools','Toys','New','A set of construction tools.',red_cresent_uuid,margeret_uuid,datetime.now().strftime("%d/%m/%Y - %H:%M:%S"),'Morning (8:00 AM - 11:00 AM)',b''))
		cur.execute('INSERT INTO items (UUID,item_name,category,condition,description,org_id,user_id,time_submitted,pickup_time,image,status) VALUES (?,?,?,?,?,?,?,?,?,?,2)', (lego_set_uuid,'Millennium Falcon Lego Set','Toys','New','A premium millennium falcon lego set.',red_cresent_uuid,donald_uuid,datetime.now().strftime("%d/%m/%Y - %H:%M:%S"),'Morning (8:00 AM - 11:00 AM)',b''))
		cur.execute('INSERT INTO items (UUID,item_name,category,condition,description,org_id,user_id,time_submitted,pickup_time,image,status) VALUES (?,?,?,?,?,?,?,?,?,?,-1)', (bedsheets_uuid,'Bedsheets','Linens','Heavily used','Some worn bedsheets.',dubai_cares_uuid,donald_uuid,datetime.now().strftime("%d/%m/%Y - %H:%M:%S"),'Morning (8:00 AM - 11:00 AM)',b''))
		cur.execute('INSERT INTO items (UUID,item_name,category,condition,description,org_id,user_id,time_submitted,pickup_time,image,status) VALUES (?,?,?,?,?,?,?,?,?,?,0)', (children_socks_uuid,'Children Socks','Clothes','New','Some colorful children socks.',dubai_cares_uuid,margeret_uuid,datetime.now().strftime("%d/%m/%Y - %H:%M:%S"),'Morning (8:00 AM - 11:00 AM)',b''))
		cur.execute('INSERT INTO items (UUID,item_name,category,condition,description,org_id,user_id,time_submitted,pickup_time,image,status) VALUES (?,?,?,?,?,?,?,?,?,?,0)', (barbie_set_uuid,'Barbie Set','Toys','Lightly used','Some barbie toys. Mostly without heads.',uae_aid_uuid,margeret_uuid,datetime.now().strftime("%d/%m/%Y - %H:%M:%S"),'Morning (8:00 AM - 11:00 AM)',b''))
		cur.execute('INSERT INTO items (UUID,item_name,category,condition,description,org_id,user_id,time_submitted,pickup_time,image,status) VALUES (?,?,?,?,?,?,?,?,?,?,3)', (software_engineering_textbook_uuid,'Software Engineering Book','Books','Lightly used','A very useful and well needed book in modern society.',red_cresent_uuid,donald_uuid,datetime.now().strftime("%d/%m/%Y - %H:%M:%S"),'Morning (8:00 AM - 11:00 AM)',b''))
		cur.execute('INSERT INTO items (UUID,item_name,category,condition,description,org_id,user_id,time_submitted,pickup_time,image,status) VALUES (?,?,?,?,?,?,?,?,?,?,-1)', (die_hard_movie_uuid,'Die Hard Movie','Books','Lightly used','An awesome christmas movie. The best ever.',red_cresent_uuid,jake_uuid,datetime.now().strftime("%d/%m/%Y - %H:%M:%S"),'Morning (8:00 AM - 11:00 AM)',b''))
		dbcon.commit()